...
public class FrontControllerTest {
    ...
    public void testNewFront() throws Exception {
        ReqNewFront param = new ReqNewFront();
        param.setFrontId(1001);
        param.setFrontIp("localhost");
        param.setFrontPort(5002);
        param.setDescription("test");

        ResultActions resultActions = mockMvc.perform(MockMvcRequestBuilders.post("/front/new")
                .content(JacksonUtils.objToString(param)).contentType(MediaType.APPLICATION_JSON));
    ...
    public void testQueryFrontList() throws Exception {
        ResultActions resultActions = mockMvc.perform(MockMvcRequestBuilders.get("/front/find"));
        resultActions.andExpect(MockMvcResultMatchers.status().isOk())
                .andDo(MockMvcResultHandlers.print());
        System.out.println(
                "response:" + resultActions.andReturn().getResponse().getContentAsString());
    ...
    public void testRemoveFront() throws Exception {
        ResultActions resultActions =
                mockMvc.perform(MockMvcRequestBuilders.delete("/front/1001"));
        resultActions.andExpect(MockMvcResultMatchers.status().isOk())
                .andDo(MockMvcResultHandlers.print());
        System.out.println(
                "response:" + resultActions.andReturn().getResponse().getContentAsString());
