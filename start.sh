...
APP_MAIN=com.webank.webase.stat.Application
...
start(){
    ...
    if [ $processStatus == 1 ]; then
        ...
        nohup $JAVA_HOME/bin/java -Djdk.tls.namedGroups="secp256k1" $JAVA_OPTS -cp $CLASSPATH $APP_MAIN >> $LOG_DIR/stat.out 2>&1 &
